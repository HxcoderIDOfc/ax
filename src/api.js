...
function withTimeout(promise, ms = 8000) {
  return Promise.race([
    promise,
    new Promise((_, reject) => setTimeout(() => reject(new Error('Axynera SDK timeout.')), ms))
  ])
}
...
async function ensureSdk() {
  ...
  if (!sdkPromise) {
    sdkPromise = withTimeout(import('./vendor/axynera.mjs'))
      .then((mod) => {
        const Axynera = mod.default
        const AxyneraAuth = mod.AxyneraAuth
        if (!Axynera || !AxyneraAuth) throw new Error('Axynera SDK v0.4.2 tidak lengkap.')
