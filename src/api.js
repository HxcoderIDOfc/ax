...
function safeAccountContext(raw={}){
  ...
  const safe={
    ...
    unlimited,
    storage:{used_bytes:storage?.used_bytes??0,limit_bytes:storage?.limit_bytes??null,remaining_bytes:storage?.remaining_bytes??null,files:storage?.files??null},
    usage:{input_tokens:usage?.input_tokens??0,output_tokens:usage?.output_tokens??0,vision:usage?.vision??0,files:usage?.files??0,search:usage?.search??0,sandbox:usage?.sandbox??0},
    limits:{input_tokens:limits?.input_tokens??null,output_tokens:limits?.output_tokens??null,vision:limits?.vision??null,files:limits?.files??null},
    remaining:{input_tokens:remaining?.input_tokens??null,output_tokens:remaining?.output_tokens??null,vision:remaining?.vision??null}
  }
...
export async function getCredits(){
  ...
  const [meResult,creditResult]=await Promise.allSettled([req(`${AUTH}/v1/me`,{headers:authHeaders(t)}),req(`${AUTH}/v1/credits`,{headers:authHeaders(t)})])
...
async function nativeFallback(messages,t,neraMode,onStage,onToken){
  ...
  if(Number(out?.status||0)<200||Number(out?.status||0)>=300)throw new Error(`API ${out?.status||'native'}`)
...
export async function streamChat(input,{onToken,onStage,mode='cepat'}={}){
  ...
  try{
    let response
    try{
      response=await fetch(`${API}/v1/chat/completions`,{method:'POST',headers:authHeaders(t,{'Content-Type':'application/json',Accept:'text/event-stream'}),body:JSON.stringify({model:MODEL,messages:conversation,stream:true,nera_events:true,nera_mode:neraMode}),signal:activeChatController.signal})
    }catch(fetchError){
      if(fetchError?.name==='AbortError')throw fetchError
      clearTimeout(timeout);activeChatController=null
      return await nativeFallback(conversation,t,neraMode,onStage,onToken)
    }
    ...
    while(true){
      ...
      while((idx=buffer.indexOf('\n\n'))>=0){
        ...
        if(eventName==='thinking'){onStage?.({id:`activity-${++eventSeq}`,label:data.status||'Berpikir',state:'active',kind:'thinking',tool:data.tool||null,command:data.command||null,success:data.success,activity:Boolean(data.activity),mode:data.mode||neraMode});continue}
        if(eventName==='status'){onStage?.({id:`status-${++eventSeq}`,label:data.status||'Menulis jawaban',state:'active',kind:'status',phase:data.phase||null,tool:data.tool||null,activity:Boolean(data.activity)});continue}
        if(eventName==='content'){const delta=data?.delta??data?.text??'';if(typeof delta==='string'&&delta){full+=delta;onToken?.(delta,full);onStage?.({id:'content',label:'Menulis jawaban',state:'active',kind:'content'})}continue}
        if(eventName==='done'){onStage?.({id:`done-${++eventSeq}`,label:data.status||'Selesai',state:'done',kind:'done'});continue}
