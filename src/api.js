...
async function nonStreamFallback(messages,t,neraMode,onStage,onToken){
  onStage?.({id:'fallback',label:'Menghubungkan ulang tanpa streaming',state:'active',kind:'thinking',activity:true})
  const data=await req(`${API}/v1/chat/completions`,{method:'POST',headers:authHeaders(t,{'Content-Type':'application/json'}),body:JSON.stringify({model:MODEL,messages,stream:false,nera_events:false,nera_mode:neraMode})},90000)
  const text=data?.choices?.[0]?.message?.content??data?.output_text??data?.text??''
  if(!text)throw new Error('Nera tidak mengirim jawaban.')
  onToken?.(text,text);onStage?.({id:'fallback-done',label:'Jawaban diterima',state:'done',kind:'done'});return text
}
...
export async function streamChat(input,{onToken,onStage,mode='cepat'}={}){
  ...
  try{
    ...
    catch(fetchError){if(fetchError?.name==='AbortError')throw fetchError;clearTimeout(timeout);activeChatController=null;return await nonStreamFallback(conversation,t,neraMode,onStage,onToken)}
    ...
    if(!response.body)return await nonStreamFallback(conversation,t,neraMode,onStage,onToken)
